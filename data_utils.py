...
class MyDataset(torch.utils.data.Dataset):
    def __init__(self, dataset, setType='train', neg_data=None):
        ...
        self.n_user = dataset['userCount']           # 1925
        self.n_item = dataset['itemCount']           # 15053
        self.n_category = dataset['categoryCount']   # 6
...
def prepare_dgl_graph(args, dataset):
    ...
    # with open('tmp.txt', 'w') as fn:
    #     for i,j in zip(uids, fids):
    #         fn.write(str(i)+' '+ str(j) +'\n')
    k = np.stack((uids, fids), axis=1).astype(np.int64)
    # a,b = 0,0
    # if (k[0][1], k[0][0]) in k:
    #     a = 1
    # if (k[0][0], k[0][1]) in k:
